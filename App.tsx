...
const App: React.FC = () => {
  ...
  const handleCapture = useCallback(async (imageData: string) => {
    ...
    try {
      ...
      speakAssessment(result.description);
    } catch (error) {
    ...
    <div className="relative w-full h-screen bg-black overflow-hidden flex flex-col">
      ...
      <header className="absolute top-0 left-0 w-full p-4 z-20 flex justify-between items-center pointer-events-none bg-gradient-to-b from-black/80 to-transparent">
        <div className="flex items-center gap-2 text-green-500 pointer-events-auto">
          ...
          <h1 className="text-xl font-bold tracking-widest uppercase">Ascii_Darshan<span className="text-xs ml-1 opacity-70">v1.0</span></h1>
        </div>
      ...
      <main className="flex-grow relative flex">
        ...
        <DiagnosticsSidebar options={options} isAnalyzing={isAnalyzing} />
      </main>
